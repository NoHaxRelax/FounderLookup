...
- Keep portfolio monitoring, follow-on investing, fund operations, exit-management UI, autonomous outreach, and actual money movement out of this MVP. The human reviewer explicitly selected a thin LangGraph outbound loop to match inbound while keeping the rest of the system framework-neutral. For the hackathon demo, the reviewer also explicitly approved OpenAI `gpt-5.6-luna` for bounded structured extraction and model-backed analysis, including founder-private inputs when the server runtime explicitly enables that data class; this approval does not make generated output authoritative or remove Evidence validation.
...
- Selects OpenAI `gpt-5.6-luna` behind the investment-intelligence seam for strict schema-constrained output, with `store=false`, explicit private-data enablement for the demo, deterministic validation/fallback, and no autonomous Decision authority; selects LangGraph only for the bounded outbound retrieval state machine.
